...
class MainPage extends React.PureComponent<Props, State> {
    ...
    plusPos: React.RefObject<HTMLDivElement>;
    ...
    constructor(props: Props) {
        ...
        this.state = {
            ...
            activeServerURL: firstServer?.url,
    ...
    reorder(reorderthis: any[], from: any[]) {
        const reordered: any[] = [];
        for(let ix = 0; ix < from.length; ix++) {
            reorderthis.map((item, _) => {
                if(item.order === from[ix].order) {
                    reordered.push(item);
                }
            })
        }
    ...
    componentDidMount() {
        ...
        this.setState({ 
            //...this.state._desktop?.teams
            // serversAndTeams: window.localStorage.getItem('DROPPABLE_ORDER') ? JSON.parse(window.localStorage.getItem('DROPPABLE_ORDER')) : [...this.props.teams, ...this.state._desktop?.teams]
            serversAndTeams: [...this.props.teams]
        });
        ...
        window.ipcRenderer.on(SERVER_TEAMS, (_, desktop) => {
            if(Object.keys(desktop).length !== 0) {
                console.log(desktop);
                this.setState({ _desktop: desktop })
            }
        });

        window.ipcRenderer.on(ALL_SERVERS_TEAMS, (_, collection) => {
            console.log('All Server Teams --->>>', collection);
        });
        ...
        window.ipcRenderer.on(SET_ACTIVE_VIEW, (event, serverName, tabName, serverURL) => {
            this.setState({activeServerName: serverName, activeTabName: tabName, activeServerURL: serverURL});
        });
    ...
    saveOrderToLocalStorage(items: any) {
        window.localStorage.setItem('DROPPABLE_ORDER', JSON.stringify(items));
    }
    ...
    render() {
                ...
                >
                    {window.process.platform !== 'linux' && this.props.teams.length === 0 && (
                        <div className='app-title'>
                            {intl.formatMessage({id: 'renderer.components.mainPage.titleBar', defaultMessage: 'Invochat'})}
                        </div>
                    )}
                    <button
        ...
        const selectedBlock = this.state.activeServerName;
        const hover = {
            in: (ev: any, ix: number) => this.setState({ hoveredButton: ix }),
            out: (ev: any, ix: number) => this.setState({ hoveredButton: -1 })
        }
        const styles = {
            selected: { transition: '0.3s', backgroundColor: 'transparent', cursor: 'pointer', color: 'white', margin: '4px 4px 4px 11px', border: `2px white solid`, padding: 5, borderRadius: 10, width: 35, textAlign: 'center' },
            hover: { transition: '0.3s', backgroundColor: 'transparent', cursor: 'pointer', color: 'white', margin: '4px 4px 4px 11px', border: `1px gray solid`, padding: 5, borderRadius: 10, width: 35, textAlign: 'center' },
            nonhover: { transition: '0.3s', backgroundColor: 'rgba(255, 255, 255, 0.16)', cursor: 'pointer', color: 'white', margin: '4px 4px 4px 11px', border: '0', padding: 6, borderRadius: 10, width: 35, textAlign: 'center' }
        }

        const blocks = {
            server: (team: any, ix: number) => {
                return (<div style={ selectedBlock === team.name ? styles.selected : selectedBlock !== team.name && this.state.hoveredButton === ix ? styles.hover : styles.nonhover } onClick={() => {
                    window.ipcRenderer.send(SWITCH_SERVER, team.name);
                }}
                    onMouseEnter={(ev) => hover.in(ev, ix)}
                    onMouseLeave={(ev) => hover.out(ev, ix)}
                >
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                        {team.name.toUpperCase().split('')[0]}
                        {team.name.toUpperCase().split('')[1]}
                    </div>
                </div>)
            },
            team: (team: any, ix: number) => {
                return (<div style={ selectedBlock === team.name ? styles.selected : selectedBlock !== team.name && this.state.hoveredButton === ix ? styles.hover : styles.nonhover } onClick={() => {
                    window.ipcRenderer.send(SWITCH_SERVER, team.name);
                }}
                    onMouseEnter={(ev) => hover.in(ev, ix)}
        ...
        return (
            <>
                <header style={{ backgroundColor: '#0a243f', padding: '22px', WebkitAppRegion: 'drag' }}>
                </header>
                <section style={{ backgroundColor: '#11375F', height: '100vh', display: 'flex', flexDirection: 'column', paddingTop: '8px', width: '54.8px', borderRight: '1px #23476c solid'}}>
                    <DragDropContext onDragEnd={res => {
                        const items = Array.from(this.props.teams);
                        const [reorderedItem] = items.splice(res.source.index, 1);
                        items.splice(res.destination?.index, 0, reorderedItem);
                        this.setState({ serversAndTeams: items });
                        this.saveOrderToLocalStorage(items);
                    }}>
                        <Droppable droppableId="servers-and-teams" type="PERSON">
                            {(provided) => (
                                <div {...provided.droppableProps} ref={provided.innerRef}>
                                    {this.props.teams.map((_team: any, idx: any) => {
                                        const relativeIndex = this.props.teams.length + idx;
                                        return (
                                            <Draggable draggableId={idx+'-ID'} key={idx} index={idx}> 
                                                {(provided) => (
                                                    <div {...provided.draggableProps} ref={provided.innerRef} {...provided.dragHandleProps}>
                                                        {blocks.team(_team, relativeIndex)}
                                                    </div>
                                                )}
                                            </Draggable>
                                        )
                    ...
                    <TeamDropdownButton
                        isDisabled={this.state.modalOpen}
                        activeServerName={this.state.activeServerName}
                        activeServerURL={this.state.activeServerName}
                        totalMentionCount={totalMentionCount}
                        hasUnreads={totalUnreadCount > 0}
                        isMenuOpen={this.state.isMenuOpen}
                        darkMode={this.state.darkMode}
