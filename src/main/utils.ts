...
export function getWindowBoundaries(win: BrowserWindow, hasBackBar = false, isLoadingScreen: boolean) {
    const {width, height} = win.getContentBounds();
    return getAdjustedWindowBoundaries(width, height, hasBackBar, isLoadingScreen);
}

export function getAdjustedWindowBoundaries(width: number, height: number, hasBackBar = false, isLoadingScreen: boolean) {
    return {
        x: isLoadingScreen ? 0 : 55,
        y: 0, //TAB_BAR_HEIGHT + (hasBackBar ? BACK_BAR_HEIGHT : 0),
        width: width - (isLoadingScreen ? 0 : 55),
