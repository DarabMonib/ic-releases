...
export class ViewManager {
    ...
    constructor(mainWindow: BrowserWindow) {
        ...
        ipcMain.on(SWITCH_SERVER, (_,serverName) => {
            this.turnHeaderDraggable();
            this.removeSidebar();
            this.fetchTeamsFromLocalStorage(this.getCurrentView()?.view, false);
        });
    ...
    makeView = (srv: MattermostServer, serverInfo: ServerInfo, tab: Tab, url?: string): MattermostView => {
        ...
        // this.fetchTeamsFromLocalStorage(this.getCurrentView()?.view, false);s
        return view;
    }
    ...
    showInitial = () => {
        ...
        if (servers.length) {
            ...
            this.mainWindow.webContents.send(SET_ACTIVE_VIEW, null, null, null);
    ...
    showByName = (name: string) => {
        ...
        if (newView) {
            ...
            newView.window.webContents.send(SET_ACTIVE_VIEW, newView.tab.server.name, newView.tab.type, newView.tab.server.url);
            ipcMain.emit(SET_ACTIVE_VIEW, true, newView.tab.server.name, newView.tab.type, newView.tab.server.url);
            if (newView.isReady()) {
    ...
    setLoadingScreenBounds = () => {
        if (this.loadingScreen) {
            this.loadingScreen.setBounds(getWindowBoundaries(this.mainWindow, false, true));
        }
    }
    ...
    fetchTeamsFromLocalStorage(view: any, initalFetch: boolean) {
        if(initalFetch) {
            view.webContents.executeJavaScript('localStorage.getItem("_desktop");', true).then((result: any)=> {
                if(result !== null) {
                    let _desktop = JSON.parse(result);
                    _desktop.teams = _desktop?.teams?.filter((value: any, index: any, self: any) =>
                        index === self.findIndex((t: any) => (
                            t.id === value.id
                        ))
                    )
                    this.mainWindow.webContents.send(SERVER_TEAMS, _desktop);
                }
            });
        } else {
            this.mainWindow.webContents.send(SERVER_TEAMS, {});
            view.webContents.executeJavaScript('localStorage.getItem("_desktop");', true).then((result: any)=> {
                if(result !== null) {
                    let _desktop = JSON.parse(result);
                    _desktop.teams = _desktop?.teams?.filter((value: any, index: any, self: any) =>
    ...
    sendAllServersTeams() {
        this.views.forEach((view) => {
            view.view.webContents.executeJavaScript(`localStorage.getItem("_desktop");`, true).then((result: any)=> {
                if(result !== null) {
                    let _desktop = JSON.parse(result);
                    _desktop.teams = _desktop?.teams?.filter((value: any, index: any, self: any) =>
                        index === self.findIndex((t: any) => (
                    ...
                    collectionsArr.push(_desktop);
                }
            });
        })
        this.mainWindow.webContents.send(ALL_SERVERS_TEAMS, collectionsArr);
    }
